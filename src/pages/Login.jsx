...
const Login = () => {
  ...
  const hanldeSubmit=()=>{
    ...
    const isValidEmail = emailRegex.test(info.email);
    if(!isValidEmail){
  ...
  const disableed=!info.name || !info.email;
  ...
  return (
    <div>
      <nav className=' flex items-center justify-between'>
        ...
        <Link to={"/"} className=' rounded-[100%]  flex items-end justify-center w-[50px] p-2 font-bold text-2xl border-2 border-gray-400'>X</Link>
      </nav>

      <h1 className='styleFont text-center mt-[75px] md:mt-[100px]'>Registration Form</h1>
      <h2 className='heroHeading text-center text-3xl md:text-5xl lg:text-6xl'>Start your success</h2>
      <h2 className='heroHeading  text-center text-3xl md:text-5xl lg:text-6xl'>Journey here!</h2>

      <div className=' flex items-center justify-center flex-col mt-8'>
        <input name='name' onChange={handleChange} type="text" placeholder='Enter your name' className=' w-full md:w-1/2 lg:w-[417px] bg-[#EFEFEF] outline-none rounded-lg p-3 px-5' />
        <input name='email' onChange={handleChange} type="email" placeholder='Enter your email' className=' w-full md:w-1/2 lg:w-[417px] bg-[#EFEFEF] outline-none mt-10 rounded-lg p-3 px-5' />
        {emailError && <p className="text-red-500 text-left">*{emailError}</p>}
        <button onClick={hanldeSubmit} disabled={disableed} className={`w-full md:w-1/2 lg:w-[417px] ${disableed?" bg-[#C9C9C9]":'bg-[#1C1C1C] hover:bg-[#4E4E4E] duration-500 ease-linear'}  text-center rounded-full  text-[16px] font-bold p-3 text-white mt-10`}>Submit</button>
      </div>
